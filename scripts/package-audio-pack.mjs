...
const approved = rows.filter((row) => row.approved === true && typeof row.reviewedAt === "string" && row.reviewedAt.length > 0);
...
for (const row of approved) {
  ...
  clips.push({ id: row.id, language: row.language, text: row.sourceText, normalizedText: row.normalizedText, url: `/audio/packs/${path.basename(outputDirectory)}/${filename}`, sha256: hash, reviewedAt: row.reviewedAt });
}
