...
export function DailyLesson({ course, current, dueIds, completedIds, onClose, onEvidence }: { course: LessonForTools[]; current: LessonForTools; dueIds: string[]; completedIds: string[]; onClose: () => void; onEvidence: (correct: boolean, language: string, lessonId: string, exercise: TranslationExercise) => void }) {
  ...
  if (screen.kind === "vocabulary") return <DailyFrame eyebrow="Today · new language" title="A small new foundation.">
    <div className="daily-vocabulary">{plan.newVocabulary.map((word) => <div key={word.word}><strong>{word.word}</strong><ListenButton text={word.word} language="es" compact/><span>English · {word.english}</span><span>Vietnamese · {word.vietnamese}</span></div>)}</div>
  ...
  if (screen.kind === "application") return <DailyFrame eyebrow="Today · application" title={plan.application.target}>
    ...
    <div className="language-stack compact-stack"><div className="stack-line"><span>English anchor</span><div><strong>{plan.application.anchor}</strong><ListenButton text={plan.application.anchor} language="en" compact/></div></div><div className="stack-line"><span>Vietnamese bridge</span><div><strong>{plan.application.bridge}</strong><ListenButton text={plan.application.bridge} language="vi" compact/></div></div></div>
